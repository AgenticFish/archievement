...
import { saveConfig, DEFAULT_CONFIG } from "../../lib/config/plugin.js";
...
test("runSessionStart injects 'unregistered' when project not in config", async () => {
  await withTmpDir(async (tmpHome) => {
    await withTmpDir(async (root) => {
      const pluginConfigPath = join(tmpHome, "plugin-data", "config.yml");
      saveConfig({ pluginConfigPath }, { ...DEFAULT_CONFIG, archievement_root: root });
      const result = await runSessionStart({
